...
var DiffApplyPlugin = class extends import_obsidian.Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      name: "\u6DF7\u5408\u7F16\u8F91\u6240\u9009\u6587\u672C",
      editorCallback: (editor, view) => this.openHybridDiffForSelection(editor, view)
    });
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor) => {
        ...
        if (selection && selection.length > 0) {
          menu.addItem(
            (item) => item.setTitle("\u6DF7\u5408\u7F16\u8F91\uFF08Hybrid Diff\uFF09").setIcon("edit").onClick(() => this.openHybridDiffForSelection(editor))
          );
        }
      })
    );
  ...
  async openHybridDiffForSelection(editor, view) {
    const selection = editor.getSelection();
    if (!selection || selection.length === 0) {
      new import_obsidian.Notice("\u8BF7\u5148\u5728\u7B14\u8BB0\u4E2D\u9009\u4E2D\u8981\u5BF9\u6BD4\u7684\u539F\u6587\u7247\u6BB5\u3002");
      return;
    }
...
var HybridDiffModal = class extends import_obsidian.Modal {
      ...
      if (index < originalLines.length) {
        ...
        if (line === originalLine) {
          ...
          if (originalLine.trim() === '' && line.trim() !== '') {
            ...
            if (hasChanges) {
              // 如果原文中这行是空的但修改版有内容，标记为新增
              if (originalLine.trim() === '') {
                lineStatus[index] = 'added';
              } else {
                lineStatus[index] = 'unchanged'; // 修改已经在原文中标记了
              }
            } else {
              lineStatus[index] = 'unchanged';
            }
  ...
  createReadOnlyEditor(container, text, isOriginal = false) {
  ...
  createEditableEditor(container, text) {
    const editor = container.createEl("textarea");
    editor.value = text;
    editor.style.width = "100%";
    editor.style.height = "100%";
    ...
    editor.style.overflow = "auto"; // 确保可以滚动
    ...
    editor.addEventListener('keydown', (e) => {
      ...
      // Cmd+Shift+Z 或 Ctrl+Shift+Z 或 Cmd+Y 或 Ctrl+Y 重做
  ...
  addHybridActions(container) {
    ...
    const fontDisplay = fontControlsContainer.createEl("span", { text: this.fontSize + "px" });
    fontDisplay.style.fontSize = "14px";
    fontDisplay.style.color = "#ccc";
    fontDisplay.style.minWidth = "40px";
    fontDisplay.style.textAlign = "center";
